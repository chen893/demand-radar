...
// 复用样式常量
const CARD_CLASS = "bg-white rounded-xl border border-gray-100 shadow-sm p-5";
const INPUT_CLASS =
  "w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all";
...
export function SettingsView() {
  ...
  const [isPromptEnabled, setIsPromptEnabled] = useState(!!systemPrompt);
  ...
  // 同步 Prompt 配置（仅在 Store 值变化且与当前不一致时更新，避免覆盖用户正在编辑的内容）
  useEffect(() => {
    console.log("systemPrompt", systemPrompt);
    // 只有当 Store 中的值存在时，才强制同步开启状态
    // 如果 Store 中为空，我们保留用户的本地状态（可能是用户刚开启但没保存，或者刚清空但没保存）
    if (systemPrompt) {
      setCustomPrompt(systemPrompt);
      setIsPromptEnabled(true);
    }
    // 注意：我们故意不处理 systemPrompt 为空的情况，以保留用户的本地操作状态
  }, [systemPrompt]);
  ...
  const handleSave = async () => {
    // LLM配置和系统Prompt是独立存储的，我们并行触发保存操作
    const saveLLM = setLLMConfig({
      provider,
      apiKey,
      baseUrl: provider === "custom" ? baseUrl : undefined,
    ...
    // 保存系统 Prompt
    const savePrompt = setSystemPrompt(isPromptEnabled ? customPrompt : "");

    try {
      await Promise.all([saveLLM, savePrompt]);
      toast.success("配置已保存");
    } catch (error) {
  ...
  const handleClearData = async () => {
    const isConfirmed = await confirm({
      title: "清空所有数据",
      message:
        "确定要清空所有数据吗？这将永久删除所有保存的产品方向和分析记录！此操作不可恢复。",
      confirmText: "确定清空",
      isDestructive: true,
    });
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
              ...
              {PROVIDERS.map((p) => (
                ...
                >
                  ...
                  <div className="flex-1 flex items-center justify-between">
                    <span
                      className={`text-sm font-medium ${provider === p.value ? "text-blue-700" : "text-gray-700"}`}
                    >
                      {p.label}
                    </span>
                    {provider === p.value && (
          ...
          <div>
            ...
            <div className="relative group">
              <input
                ...
                className={`${INPUT_CLASS} pl-3 pr-10 text-gray-800 placeholder-gray-400`}
              />
              <button
                type="button"
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
              ...
              <span className="text-gray-400 font-normal normal-case">
                (默认为 {getProviderDefaultModel(provider) || "gpt-4o-mini"})
              </span>
            </label>
            <input
              type="text"
        ...
        {/* Analysis Tuning Card */}
        <div className={`${CARD_CLASS} space-y-4`}>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className="text-xl">🎛️</span>
              <h2 className="font-semibold text-gray-900">分析调优</h2>
            </div>

            <div className="flex items-center gap-2">
              <span
                className={`text-xs font-medium ${isPromptEnabled ? "text-brand-600" : "text-gray-400"}`}
              >
                {isPromptEnabled ? "已启用" : "使用默认"}
              </span>
              <button
                ...
                className={`
                            ...
                            ${isPromptEnabled ? "bg-brand-600" : "bg-gray-200"}
                        `}
              >
                <span
                  className={`
                                pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out
                                ${isPromptEnabled ? "translate-x-4" : "translate-x-0"}
                            `}
                />
              </button>
            </div>
          ...
          <div
            className={`transition-all duration-300 ${isPromptEnabled ? "opacity-100 max-h-[500px]" : "opacity-60 max-h-12 overflow-hidden blur-[1px]"}`}
          >
            <div className="space-y-3">
              ...
              <div className="relative">
                <textarea
                  ...
                  onBlur={() => {
                    setSystemPrompt(isPromptEnabled ? customPrompt : "");
                  }}
                  disabled={!isPromptEnabled}
                  placeholder="例如：请侧重关注 B2B SaaS 领域的痛点，忽略针对个人消费者的讨论。分析时请使用更专业、简洁的中文..."
                  className="w-full h-32 px-3 py-2.5 bg-slate-50 border border-gray-200 rounded-lg text-xs font-mono text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all resize-none disabled:bg-gray-100 disabled:text-gray-400"
                />
                {isPromptEnabled && (
                  <div className="absolute bottom-2 right-2 flex items-center gap-1 text-[10px] text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100">
                    <span>⚠️</span>
                    <span>将追加到默认 Prompt 末尾</span>
                  </div>
                )}
              </div>

              <p className="text-[10px] text-gray-400 leading-relaxed">
                您的指令将作为额外约束条件注入到系统核心 Prompt
                中。请谨慎修改，错误的指令可能导致分析质量下降。
              </p>
            </div>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden border border-gray-100">
                  <div
                    ...
                    style={{
                      width: `${Math.min(storageUsage.percentage * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
